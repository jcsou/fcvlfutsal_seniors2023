            ...
            <v-col cols="12" sm="6" lg="4">
                <v-card class="mx-auto px-0" max-width="400">
                  ...
                  <v-card-text class="text-center pa-1">
                    ...
                    <p class="font-weight-light grey--text">Temps de jeu 6 minutes, Rotation toutes les 7 minutes</p>
                  </v-card-text>
                ...
                <base-material-card
                    ...
                    >
                      <v-simple-table>
                        ...
                        <tbody>
                          <tr v-for="(equipe, n) in equipesABC" :key="equipe.id">
                            <td>{{n+1}}</td>
                            <td>{{equipe.poule}}</td>
                      ...
                      <v-card-actions class="pa-0 ">
                        ...
                        <v-spacer></v-spacer>
                        <v-chip class="ma-2" label link to="/PoulesABC">
                ...
                <base-material-card
                    ...
                    >
                      <v-simple-table>
                        ...
                        <tbody>
                          <tr v-for="(equipe, n) in equipesDEF" :key="equipe.id">
                            <td>{{n+1}}</td>
                            <td>{{equipe.poule}}</td>
                            <td><v-img  v-bind:src="'img/fanion/'+equipe.fanion" max-height="30" max-width="30"/></td>
                            <td>{{equipe.nom}}<v-spacer></v-spacer></td>
                      ...
                      <v-card-actions class="pa-0 ">
                          ...
                          <v-spacer></v-spacer>
                        <v-chip class="ma-2" label link to="/PoulesDEF">
                           <v-icon left>mdi-view-split-vertical</v-icon> Poules
                        </v-chip>
                      </v-card-actions>
                    </base-material-card>
    ...
    methods: {
        ...
        loadDataOds() {
            ...
            axios
                .get(url)
                .then(response => {
                  this.equipesABC = response.data.lesequipes.filter(function (entry){return entry.categorie_id==='c1';})
                  this.equipesDEF = response.data.lesequipes.filter(function (entry){return entry.categorie_id==='c2';})
                }).catch(error => {
                   console.log(error)
                })
          },
